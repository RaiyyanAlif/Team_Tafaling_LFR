...
//                      (original behavior).
// ═══════════════════════════════════════════════════════════════
enum PriorityMode { PRIORITY_LEFT, PRIORITY_RIGHT, PRIORITY_FRONT };
...
// ─────────────────────────────────────────────────────────────
//  drawPriorityMenu() — Left / Right / Front selection
// ─────────────────────────────────────────────────────────────
...
void selectPriorityMenu() {
  ...
  while (true) {
    ...
    // ── BTN_START tap: cycle Left -> Right -> Front -> Left ─
    if (digitalRead(BTN_START) == LOW) {
      while (digitalRead(BTN_START) == LOW) delay(5);
      delay(30);
...
digitalWrite(SLP, HIGH);
  ...
  // ★ NEW: choose fallback priority — Left / Right / Front ★
  selectPriorityMenu();
...
void loop() {
  ...
  if (isJunction()) {
    ...
    // ── PRIORITY-based junction behavior (string exhausted / none set) ──
  ...
  if (pos == -1) {
    ...
    if (priorityMode == PRIORITY_LEFT) {
      ...
      // PRIORITY_FRONT: fall back to previous-direction memory (original behavior)
      if (lastDir > 0) {
        motorLeft(SEARCH_SPEED, true);
        motorRight(SEARCH_SPEED, false);
      } else {
