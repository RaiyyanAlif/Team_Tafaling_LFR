...
// ─────────────────────────────────────────────────────────────
//  MODE TOGGLE:
//    Automatic: while running, the sensor pattern is continuously
//    evaluated and INVERTED_TRACK flips INSTANTLY (with a tiny
//    2-sample noise guard) when the line color scheme changes.
//
//    Manual override (still available), while robot is STOPPED:
//      → Short press BTN_STOP (< LONG_PRESS_MS)   : toggle NORMAL ↔ INVERTED
//      → Long  press BTN_STOP (≥ LONG_PRESS_MS)   : enter PATH SOURCE menu
...
// ═══════════════════════════════════════════════════════════════
//  ★  TRACK MODE  —  AUTO DETECTED (startup + continuously live)  ★
// ═══════════════════════════════════════════════════════════════
bool INVERTED_TRACK = false;   // auto-set in calibrate(); kept live-updated in autoDetectTrackMode()
...
// ── Speed ───────────────────────────
int BASE_SPEED   = 130;   // now selectable at startup from {100,110,...,160}
int MAX_SPEED    = 255;
int MOTOR_MIN    = 80;
int SEARCH_SPEED = 130;

// ═══════════════════════════════════════════════════════════════
//  ★  BASE SPEED MENU  —  select 100..160 (step 10) at startup  ★
// ═══════════════════════════════════════════════════════════════
#define NUM_SPEED_OPTIONS 7
const int SPEED_OPTIONS[NUM_SPEED_OPTIONS] = {100, 110, 120, 130, 140, 150, 160};
int speedOptionIndex = 3;   // default -> 130 (middle option)
...
int JUNCTION_COUNT = 5;   // updated by calibrate(), toggleMode(), and autoDetectTrackMode()
...
//           the bot falls back to NORMAL line-following logic
//           (no forced turns) — see loop().
//
//  The active path can come from either:
//    • a preloaded CHECKPOINT (see CHECKPOINT PATHS below), or
//    • a MANUALLY typed-in string via the live editor.
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
//  ★  CHECKPOINT PATHS  —  up to 8 preloaded path strings  ★
//  Edit the placeholders below with your own saved routes.
// ═══════════════════════════════════════════════════════════════
#define NUM_CHECKPOINTS 8
const char* checkpointPaths[NUM_CHECKPOINTS] = {
  "LLFFRLLF",   // checkpoint 1
  "LFLLFF",     // checkpoint 2
  "",           // checkpoint 3 (placeholder - edit as needed)
  "",           // checkpoint 4 (placeholder - edit as needed)
  "",           // checkpoint 5 (placeholder - edit as needed)
  "",           // checkpoint 6 (placeholder - edit as needed)
  "",           // checkpoint 7 (placeholder - edit as needed)
  ""            // checkpoint 8 (placeholder - edit as needed)
};

// ═══════════════════════════════════════════════════════════════
//  ★  LIVE AUTO MODE-DETECTION  —  no button needed  ★
// ═══════════════════════════════════════════════════════════════
//  Every loop, readSensors() grabs raw ADC values and asks
//  autoDetectTrackMode() whether the CURRENT interpretation
...
//  scheme just changed — so we switch modes basically instantly
//  (only a couple of confirming samples, to shrug off single-frame
//  noise), rather than waiting many loops. This avoids the robot
//  "losing the line" and spinning/backing up while it waits to
//  decide to switch.
//
//  Cross junctions / all-black stop bars make BOTH interpretations
//  look invalid (0 or 8 sensors lit) — so they never trigger a
//  false switch.
// ═══════════════════════════════════════════════════════════════
...
void showReady() {
  ...
  display.println("Hold STOP=path menu");
  display.display();
}

// ─────────────────────────────────────────────────────────────
//  toggleMode()  — manual override: flip INVERTED_TRACK, update
//  dependants, flash confirmation on OLED.
//  Called from loop() when BTN_STOP is short-pressed while stopped.
//  (Kept for manual override even though auto-detection now runs
//  continuously while the robot is moving.)
// ─────────────────────────────────────────────────────────────
void toggleMode() {
  INVERTED_TRACK = !INVERTED_TRACK;
...
void drawSpeedMenu(int idx) {
  ...
  for (int i = 0; i < NUM_SPEED_OPTIONS; i++) {
...
//    BTN_START (D10) tap  : cycle 100 -> 110 -> ... -> 160 -> 100 ...
...
void selectBaseSpeedMenu() {
  ...
  while (true) {
    ...
    if (digitalRead(BTN_START) == LOW) {
      ...
      speedOptionIndex++;
      if (speedOptionIndex >= NUM_SPEED_OPTIONS) speedOptionIndex = 0;
...
// ─────────────────────────────────────────────────────────────
//  drawPathSourceMenu() — choose between CHECKPOINTS and MANUAL
// ─────────────────────────────────────────────────────────────
void drawPathSourceMenu(int idx) {
  display.clearDisplay();
  display.setCursor(0, 0);
  display.println("PATH SOURCE");
  display.println("");

  const char* opts[2] = {"Checkpoints", "Manual"};
  for (int i = 0; i < 2; i++) {
    if (i == idx) display.print("> ");
    else          display.print("  ");
...
// ─────────────────────────────────────────────────────────────
//  drawCheckpointMenu() — cycle through preloaded checkpoints
// ─────────────────────────────────────────────────────────────
void drawCheckpointMenu(int idx) {
  ...
  display.println("");
  display.print("Str: ");
  if (strlen(checkpointPaths[idx]) == 0) {
    display.println("(empty)");
  } else {
    display.println(checkpointPaths[idx]);
  }
  display.println("");
  display.println("TAP D10 = next");
  display.println("HOLD D12 = select");
...
// ─────────────────────────────────────────────────────────────
//  loadCheckpoint() — copies a preloaded checkpoint string into
//  the active pathString/pathLength used by loop().
// ─────────────────────────────────────────────────────────────
void loadCheckpoint(int idx) {
  strncpy(pathString, checkpointPaths[idx], PATH_MAX_LEN);
...
// ─────────────────────────────────────────────────────────────
//  selectCheckpointMenu() — interactive menu using:
//    BTN_START (D10) tap  : cycle through checkpoint 1..8
//    BTN_STOP  (D12) hold : confirm current checkpoint -> load it
// ─────────────────────────────────────────────────────────────
void selectCheckpointMenu() {
  int cpIndex = 0;
  drawCheckpointMenu(cpIndex);
  ...
  while (true) {
    ...
    if (digitalRead(BTN_START) == LOW) {
      ...
      cpIndex++;
      if (cpIndex >= NUM_CHECKPOINTS) cpIndex = 0;
    ...
    if (digitalRead(BTN_STOP) == LOW) {
      ...
      if (heldMs >= LONG_PRESS_MS) {
        loadCheckpoint(cpIndex);
        return;   // done — pathString/pathLength already set, READY shown
      } else {
        // short press while browsing checkpoints: ignored
        drawCheckpointMenu(cpIndex);
      }
    }
  }
...
// ─────────────────────────────────────────────────────────────
//  selectPathSourceMenu() — entry point for path programming.
//  Lets the user pick CHECKPOINTS (preloaded) or MANUAL (live
//  editor). Called from loop() on a long BTN_STOP press while
//  the robot is stopped.
// ─────────────────────────────────────────────────────────────
void selectPathSourceMenu() {
  int srcIndex = 0;   // 0 = Checkpoints, 1 = Manual
  drawPathSourceMenu(srcIndex);

  while (true) {

    // ── BTN_START tap: cycle Checkpoints <-> Manual ─────────
    if (digitalRead(BTN_START) == LOW) {
      while (digitalRead(BTN_START) == LOW) delay(5);
      delay(30);
    ...
    if (digitalRead(BTN_STOP) == LOW) {
      ...
      if (heldMs >= LONG_PRESS_MS) {
        if (srcIndex == 0) {
          selectCheckpointMenu();   // shows its own confirm + READY
        } else {
          enterProgramPathMode();   // shows its own confirm + READY
        }
        return;
      } else {
        // short press while choosing source: ignored
        drawPathSourceMenu(srcIndex);
      }
    }
  }
...
void setup() {
  ...
  // ★ select base speed (100..160) before going READY ★
  selectBaseSpeedMenu();
...
void loop() {
  ...
  if (digitalRead(BTN_STOP) == LOW) {
    ...
    if (running) {
      ...
      if (pressDuration >= LONG_PRESS_MS) {
        // Long press while stopped = choose path source (Checkpoints/Manual)
  ...
  if (isJunction()) {
    ...
    // Only intervene at a junction if we STILL have un-consumed
    // letters in the programmed path. Once the path is exhausted
    // (or none was ever set), fall straight through — no forced
    // turn — so the robot follows the line with normal PID /
    // centerActive logic below, just like a fresh line-follower.
    ...
    // ── Path exhausted / no path set: NO forced turn here.
    // Intentionally fall through to centerActive()/PID below,
    // so the bot behaves like a normal line follower. ──
  }
